...
async def scrapling_maps_scraper(
    query: str,
    max_results: int = 10,
) -> list[dict]:
    """
    Scrape Google Maps using Scrapling's AsyncStealthySession (no binary subprocess).
    Much faster — results in ~10-15 seconds instead of minutes.
    """
    ...
    logger.info(f"[ScraplingMaps:{run_id}] Fetching {search_url}")
    ...
    try:
        ...
        if not result or result.status >= 400:
            logger.error(f"[ScraplingMaps:{run_id}] Failed: status={result.status if result else 'None'}")
            return []
        ...
        businesses = []
        result_items = sel.css('div[role="feed"] > div, div.section-result, a[href*="/maps/place/"]')

        if not result_items:
            result_items = sel.css('a[href*="maps/place"]')
        ...
        for item in result_items:
            ...
            try:
                text = item.get_all_text(strip=True)[:300]
                if not text or len(text) < 10:
                    continue

                href = item.attrib.get("href", "") if hasattr(item, "attrib") else ""
                item_sel = Selector(content=item.html if hasattr(item, "html") else "")

                name_el = item_sel.css('div.fontHeadlineSmall, span.fontHeadlineSmall, h3, div[role="heading"]').first
                business_name = name_el.get_all_text(strip=True) if name_el else ""

                if not business_name:
                    lines = [l.strip() for l in text.split("\n") if l.strip()]
                    business_name = lines[0] if lines else ""
                ...
                if not business_name or business_name == "Google Maps":
                    continue
                ...
                businesses.append({
                    ...
                    "google_maps_link": href if href.startswith("http") else f"https://www.google.com/maps{href}" if href.startswith("/") else "",
                    "photos": [],
                    "business_hours": {},
        ...
        logger.info(f"[ScraplingMaps:{run_id}] Extracted {len(businesses)} businesses")
        return businesses

    except Exception as e:
        logger.error(f"[ScraplingMaps:{run_id}] Error: {e}")
        return []
    finally:
        try:
            await session.close()
